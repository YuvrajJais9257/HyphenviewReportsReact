  ...
  console.log(transformedData,"transformedData");
    ...
    const addTable = (table) => {
        setSelectedTables((prevSelectedTables) => {
          ...
          console.log(table,"isTableSelected")
    ...
    const handleOrderByDirectionChange = (index, order) => {
        ...
        //         const existingSelection = prevSelections.find(item => item.column === column);
        
        //         if (existingSelection) {
        //             if (column) {
        //                 // Remove the selection if a column is specified
        //                 return prevSelections.filter(item => item.column !== column);
        //             } else {
        //                 // Clear all selections if 'COUNT(*)' is selected without a column
        //                 return [];
        //             }
        //         } else {
        //             // Add the new selection
        //             return [
        //                 ...prevSelections.filter(item => item.column !== undefined), // Remove any previous selection without a column
    ...
    const handleGroupByColumnChangeforSQLFunction = (column) => {
        const isSelected = sqlFunctionsByColumn.find(item => item.column === column);
        if (isSelected) {
            setSqlFunctionsByColumn(prevSelections => prevSelections.filter(item => item.column !== column));
        } else {
        setSqlFunctionsByColumn(prevSelections => [
            ...prevSelections,
            { column, func: undefined }
...
//     setSqlFunctionsByColumn(prevSelections => {
//         const updatedSelections = [...prevSelections];
//         // Check if the object at the given index exists before updating its properties
...
const handleSqlFunctionChange = (index, func) => {
    setSqlFunctionsByColumn(prevSelections => {
        const updatedSelections = [...prevSelections];
        if (func === 'COUNT(*)') {
            // If 'Count(*)' is selected, set the column to undefined
            updatedSelections[index] = { column: undefined, func: 'COUNT(*)' };
        } else {
            updatedSelections[index].func = func;
        }
        return updatedSelections;
    ...
    console.log(commonColumns,"common")
    ...
    const buildJoinClause = (joinClauses) =>{
        ...
        var joinClauseString = "";
            if (joinClauses[0].value === ("INNER JOIN" || "RIGHT JOIN" || "LEFT JOIN" || "FULL JOIN")) {
                joinClauseString += " " + joinClauses[0].value + " " + joinClauses[0].operator.split(".")[0] + " " + "ON" + " " + joinClauses[0].column + "=" + joinClauses[0].operator;
                console.log(joinClauseString,joinClauses,"joins")
            }
    ...
    const handleGenerateQuery = () => {
        ...
        if (sqlFun.length > 0) {
            ...
            // } else {
            //     // Handle other SQL functions where a column is associated
            ...
            if (sqlFun !== "Select SQL Function" && sqlFun.length == 1) {
                ...
                else if (sqlCol.length === 0 && sqlFun === 'COUNT(*)') {
                    console.log("Pushing COUNT(*) directly.");
                    sqlFunctions.push("COUNT(*)");
                }
            }
        ...
        console.log(joinConditions,"parts1")
        ...
        query += uniqueParts.map((table)=>table).join(', ');
            ...
            console.log(selectedColumns,"hii")
          query += selectedColumns.map((column)=>column).join(', ');
           ...
           if(joinClause.length == 1 && joinClause[0] == "JOIN"){
            ...
            console.log(selectedColumns,"hii2")
            query += uniqueParts.map((table)=>table).join(', ');
           }
        }
        ...
        if(joinConditions.length>0){
            ...
            if(joinClauses.length>0 && (whereConditions.length == 0 && sqlFun.length == 0 && orderByColumnDirection.length == 0)){
                var joinClauseString = buildJoinClause(joinClauses);
                    query += joinClauses[0].column + ", " + joinClauses[0].operator + " FROM " + joinClauses[0].column.split('.')[0] + joinClauseString;
                    console.log(query,"snap")
            }
            else {
                var joinClauseString = buildJoinClause(joinClauses);
                query += joinClauseString;
                console.log(query,"queryshown")
            }
        }
                ...
                <div className='mainConatiner'>
                    ...
                    <div className='BuildQuery_right'>
                        <div className='cardConatiner' style={{ display: 'block' }}>
                            {selectedTables.map((table) => (<div className='QueryBuilder_card' key={table.name}>
                                        ...
                                        {activeButton === 'SQL Functions' && (
                                            <div className="sql-functions-section">
                                                ...
                                                <select
                                                    ...
                                                    size="3"
                                                >
                                                    <option value="" disabled>Select SQL Function</option>
                                                    {console.log(selectedSqlFunction,"selectedfunction")}
                                                    {sqlFunctions.map((func, index) => (
                                                    <option  key={index}  value={func}
                                                    selected={sqlFunctionsByColumn[sqlFunctionsByColumn.length - 1]?.func === func} // Highlight the selected option
                                                        >
                                                            {func}
                                                        </option>
                                                    ))}
