...
export default function UpdateReportPage(props) {
  ...
  useEffect(() => {
    if (report_id) {
      dispatch(getReportDetailByID({ database_type: "mysql", report_id: report_id }));
  ...
  useEffect(() => {
    ...
    if (report_id === reortdata?.report_id.toString() &&  reortdata?.report_type === 'Box') {
    ...
    }else if(report_id === reortdata?.report_id.toString() &&  reortdata?.report_type === 'Table'){
      setformdata((prevData) => ({
        ...prevData,
        title: reortdata?.report_template_name,
        query: reortdata?.defined_query,
    ...
    }else if(report_id === reortdata?.report_id.toString() &&  reortdata?.report_type === 'Chart'){
      setformdata((prevData) => ({
        ...prevData,
        title: reortdata?.report_template_name,
        query: reortdata?.defined_query,
  ...
  const handelTestQuery = async (e) => {
    ...
    if(reportdetail?.data?.rdbms_name && reportdetail?.data?.schema_name){
      console.log(reportdetail?.data?.rdbms_name,reportdetail?.data?.schema_name,"reportdetail?.data?.rdbms_name")
      dispatch(testquryonCustompage({ query: formdata.query, schema: reportdetail?.data?.schema_name, email: user.user_email_id, connection_type: reportdetail?.data?.rdbms_name, database_type: "mysql" }))
    }
  }
                ...
                <div className="radio col-md-2">
                  <label className="radio-inline">
                    <input
                      ...
                      disabled={!formdata.query.toLowerCase().includes("group by")&&(!formdata.type==="Box")}
                    />{" "}
                    Yes
                  </label>
                  {console.log(formdata.query.toLowerCase().includes("group by"))}
                  <label className="radio-inline">
                    <input
                      type="radio"
                      name="enableDrilldown"
